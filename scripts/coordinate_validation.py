...
def main():
    ...
    # Validations 6/7: round trip in logical pixels, sample fixed seed.
    sample = random.Random(7).sample(all_points, min(1000, len(all_points)))
    errors = []
    for map_id, x, z in sample:
    ...
    text += markdown_table(["Check", "Result"], [
        ("Invalid/non-finite X or Z", invalid), ("1,000-point world→pixel→world RMSE", f"{np.sqrt(np.mean(np.square(errors))):.12g} world units"),
        ("Maximum error", f"{max(errors):.12g} world units"), ("Mean error", f"{np.mean(errors):.12g} world units"),
    ])
    ...
    print("Wrote docs/coordinate-validation.md and 9 validation images.")
