...
export default function App() {
  ...
  useEffect(() => { setTime(0); setPlaying(false); setSelectedPlayerId(null); setSelectedEvent(null); setArea(null); }, [data.match?.matchId]);
  ...
  return <main className="app-shell"><header className="topbar"><div className="brand"><Compass size={20} /><span>ATLAS</span><small>PLAYER JOURNEY EXPLORER</small></div><div className="topbar-status"><i /> Static telemetry workspace <HelpCircle size={16} /></div><IconButton label="Collapse supporting panels"><PanelLeftClose size={17} /></IconButton></header><div className="workspace">
    ...
    <section className="map-workspace"><MapCanvas match={data.match} state={data.matchState} error={data.matchError} playbackTime={time} selectedPlayerId={selectedPlayerId} selectedEvent={selectedEvent} area={area} heatmap={heatmap} heatmapOpacity={opacity} layers={layers} onHoverEvent={setHoveredEvent} onSelectEvent={setSelectedEvent} onSelectPlayer={setSelectedPlayerId} onArea={setArea} /><PlaybackBar duration={duration} time={time} playing={playing} speed={speed} disabled={data.matchState !== "ready"} onPlay={() => setPlaying((current) => !current)} onRestart={() => { setPlaying(false); setTime(0); }} onTime={(next) => { setPlaying(false); setTime(next); }} onSpeed={setSpeed} /></section>
    <InspectorPanel match={data.match} selectedPlayerId={selectedPlayerId} selectedEvent={selectedEvent} hoveredEvent={hoveredEvent} area={area} onPlayer={setSelectedPlayerId} onClearArea={() => setArea(null)} />
  </div></main>;
}
